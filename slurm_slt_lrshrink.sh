...
# --- cosine restarts with lr_shrink=0.75 (each cycle peaks at 75% of previous) ---
ENC_LAYERS=6
DEC_LAYERS=3
LR=1e-3
DP=0.3
...
WANDB_NAME=agcn_enc${ENC_LAYERS}_dec${DEC_LAYERS}_lrshrink${LR_SHRINK}
...
apptainer exec "$IMAGE" bash -lc "
    ...
    python -m fairseq_cli.hydra_train \
        ...
        lr_scheduler.lr_shrink=$LR_SHRINK
" >> "$OUTPUT_FILE" 2>&1
EXIT_CODE=$?
