#!/usr/bin/env bash
# Train the Tarrés baseline_6_3 Transformer on our 128-d AGCN SSL-projection-head
# features (ablation vs the 256-d backbone features in run_train_agcn.sh).
set -e
...
WANDB_NAME=agcn_proj128_6_3_new \
CUDA_VISIBLE_DEVICES=0 \
PYTHONPATH=$REPO \
$SLT/fairseq-hydra-train \
  --config-dir $CFG \
  --config-name agcn_signrep_6_3_proj128.yaml
