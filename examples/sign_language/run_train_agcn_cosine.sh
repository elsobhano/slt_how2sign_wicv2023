#!/usr/bin/env bash
# Separate experiment: 256-d AGCN features with SINGLE cosine annealing
# (one cosine decay over the whole run, no warm restarts). Runs from scratch
# in its own save dir, independent of the warm-restart baseline.
set -e
...
WANDB_NAME=agcn_signrep_6_3_cosine \
CUDA_VISIBLE_DEVICES=0 \
PYTHONPATH=$REPO \
$SLT/fairseq-hydra-train \
  --config-dir $CFG \
  --config-name agcn_signrep_6_3_cosine.yaml
