...
#SBATCH --job-name="mc_w12_s2_lr5e-3"
...
WANDB_NAME=agcn_meanconf_${TAG}_lr5e-3
...
apptainer exec "$IMAGE" bash -lc "
    ...
    python -m fairseq_cli.hydra_train \
        ...
        model.encoder_layers=$ENC_LAYERS model.decoder_layers=$DEC_LAYERS lr_scheduler.lr=[$LR]
" >> "$OUTPUT_FILE" 2>&1
EXIT_CODE=$?
