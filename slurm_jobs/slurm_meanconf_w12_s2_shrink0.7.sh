...
#SBATCH --job-name="mc_w12_s2_shrink0.7"
...
WANDB_NAME=agcn_meanconf_${TAG}_shrink0.7
...
apptainer exec "$IMAGE" bash -lc "
    ...
    python -m fairseq_cli.hydra_train \
        ...
        model.encoder_layers=$ENC_LAYERS model.decoder_layers=$DEC_LAYERS lr_scheduler.lr_shrink=0.7
" >> "$OUTPUT_FILE" 2>&1
EXIT_CODE=$?
