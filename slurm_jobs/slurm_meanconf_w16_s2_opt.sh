...
# --- feature set: SAME as the baseline w16_s2 run, for a clean A/B ---
TAG=w16_s2
...
# _opt suffix -> shows up as a separate wandb run next to agcn_meanconf_w16_s2.
WANDB_NAME=agcn_meanconf_${TAG}_opt
...
echo "Opt override: wd=1e-4, single-cosine anneal to 1e-6 (no restarts)" >> "$OUTPUT_FILE"
...
# Optimizer/schedule fix vs the baseline:
#   optimizer.weight_decay 1e-1 -> 1e-4        (baseline over-regularised)
#   lr_scheduler.lr_period_updates 17000 -> MAX_UPDATE  (single cosine, NO warm restarts)
#   lr_scheduler.min_lr 1e-4 -> 1e-6           (actually anneal low instead of a jagged sawtooth)
apptainer exec "$IMAGE" bash -lc "
    ...
    python -m fairseq_cli.hydra_train \
        ...
        lr_scheduler.lr_period_updates=$MAX_UPDATE lr_scheduler.min_lr=1e-6
" >> "$OUTPUT_FILE" 2>&1
EXIT_CODE=$?
