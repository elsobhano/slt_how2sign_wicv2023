...
#SBATCH --job-name="mc_w16_s2_ls0.2"
...
WANDB_NAME=agcn_meanconf_${TAG}_ls0.2
...
apptainer exec "$IMAGE" bash -lc "
    ...
    python -m fairseq_cli.hydra_train \
        ...
        model.encoder_layers=$ENC_LAYERS model.decoder_layers=$DEC_LAYERS criterion.label_smoothing=0.2
" >> "$OUTPUT_FILE" 2>&1
EXIT_CODE=$?
