...
# --- single cosine decay over full run, no restarts ---
ENC_LAYERS=6
DEC_LAYERS=3
LR=1e-3
DP=0.3
MAX_UPDATE=100000
LR_PERIOD=$MAX_UPDATE   # one full cosine cycle = no restarts

# --- fixed settings ---
CONFIG=agcn_signrep_6_3.yaml
FEATURES=agcn_features
WANDB_PROJECT=how2sign-slt-agcn
WANDB_NAME=agcn_enc${ENC_LAYERS}_dec${DEC_LAYERS}_single_cosine
...
apptainer exec "$IMAGE" bash -lc "
    ...
    python -m fairseq_cli.hydra_train \
        ...
        lr_scheduler.lr_period_updates=$LR_PERIOD
" >> "$OUTPUT_FILE" 2>&1
EXIT_CODE=$?
